...
Password1234
##################
#ßFor your specific case, I usually use the following when purging and reinstalling my env:
sudo snap install microk8s --channel=1.21/stable --classic
#2	add microk8s to kube config, for example with 
microk8s config > ~/.kube/config 
#(not that will overwrite your existing one)
microk8s enable rbac dns storage metallb:10.64.140.43-10.64.140.49
#	(waits for hostpath to deploy, this way you can run all the commands in a script) 
microk8s kubectl rollout status deployment/hostpath-provisioner -n kube-system
juju bootstrap --no-gui microk8s uk8sx 
#<-- I think this was a bit different than yours - this uses the fact that juju 
#knows how to interact with microk8s rather than treating it as a vanilla k8s cluster
...
microk8s enable rbac gpu dns dashboard storage ingress metallb:10.64.140.43-10.64.140.49
